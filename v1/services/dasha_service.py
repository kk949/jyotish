...
def get_yogini_main(dob, tob, lat, lon, tz):
    d1, bd = _prepare(dob, tob, lat, lon, tz)
    d1_fixed = copy.deepcopy(d1)
    moon = d1_fixed.get("planets", {}).get("Moon", {})
    if "nakshatra_name" not in moon and "nakshatra" in moon:
        moon["nakshatra_name"] = moon["nakshatra"]
    periods = getYoginiDashaMain(d1_fixed, bd)
    for p in periods:
        p["start_date"] = str(p["start_date"])
        p["end_date"]   = str(p["end_date"])
    return periods
...
def get_yogini_sub(dob, tob, lat, lon, tz, lord: str):
    ...
    if "nakshatra_name" not in moon and "nakshatra" in moon:
        moon["nakshatra_name"] = moon["nakshatra"]
    sub = getYoginiDashaSub(d1_fixed, bd, lord)
    if isinstance(sub, list):
        for p in sub:
            if "start_date" in p: p["start_date"] = str(p["start_date"])
            if "end_date"   in p: p["end_date"]   = str(p["end_date"])
