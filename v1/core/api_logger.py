    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                # Dev/CI friendliness: allow the app to run without a DB.
                # We still require api_id to be present (handled above), but we
                # skip auth + logging when the pool isn't initialized.
                logger.error(f"Pre-call DB unavailable for {endpoint_name}: {e}")
                log_id = None
                user_id = None
