...
def function(point):
    ...
    #longi, flags = swe.fixstar_ut("Revati", point, swe.FLG_SWIEPH | swe.FLG_SIDEREAL)
    #fval = norm180(longi[0]) - ((359 + 49/60 + 59/3600) - 360)
...
def sunrise(jd, place):
  ...
  lat, lon, tz = place
  geopos = (lon, lat, 0)  # (longitude, latitude, height)
  
  # Set flags for calculation
  rsmi = swe.CALC_RISE | swe.BIT_DISC_CENTER
  
  # Call swe.rise_trans with parameters in the correct order for latest pyswisseph
  # Using only the required parameters (7 max)
  result = swe.rise_trans(
      ...
      0,            # atpress (default)
      0,            # attemp (default)
      swe.FLG_SWIEPH # epheflag
  )


  # Check the status code. The function returns a tuple with status and rise time
  res_code = result[0]
  if res_code >= 0:  # Success
      # result[1][0] is the Julian day number in UT
      rise = result[1][0]
      # Convert to local time and return
      rise_local = rise + tz/24.0
      # Assuming a to_dms function is defined elsewhere for formatting.
      # Placeholder for now.
      def to_dms(time):
          hours = int(time)
      ...
      return [rise_local, to_dms((rise_local - int(rise_local)) * 24)]
  else:
      # An error occurred (e.g., circumpolar)
      print(f"Error calculating sunrise. Status code: {res_code}")
      return [None, None]

year, month, day = 2025, 10, 4
hour_val = 11 + 21/60 + 0/3600

# Call swe.julday with the correct number of arguments
jd = swe.julday(year, month, day, hour_val)
place = (22.2989, 70.7958, 5.5) # Rajkot, India
rise_time = sunrise(jd, place)

if rise_time:
    print(f"Sunrise Julian Day (Local): {rise_time[0]}")
    print(f"Sunrise Time (HH:MM:SS): {rise_time[1]}")

def sunset(jd, place):
  """Sunset when centre of disc is at horizon for given date and place"""
  lat, lon, tz = place
  geopos = (lon, lat, 0)
  rsmi = int(_rise_flags + swe.CALC_SET)
  result = swe.rise_trans(
      jd - tz/24,
      swe.SUN,
      rsmi,
...
def moonrise(jd, place):
  ...
  lat, lon, tz = place
  geopos = (lon, lat, 0)
  rsmi = int(_rise_flags + swe.CALC_RISE)
  result = swe.rise_trans(
      jd - tz/24,
      swe.MOON,
      rsmi,
...
def ascendant(jd, place):
  ...
  nirayana_lagna = swe.houses_ex(jd_utc, lat, lon, b'P')[1][0]
  # 12 zodiac signs span 360°, so each one takes 30°
  # 0 = Mesha, 1 = Vrishabha, ..., 11 = Meena
